...
import { Calendar, MapPin, Share2, Play, FileText, DollarSign, MessageCircle, Heart, Users, Phone } from "lucide-react"
...
export function FuneralEventPage({ funeral }: FuneralEventPageProps) {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
          ...
          <div className="flex flex-col lg:flex-row items-start space-y-8 lg:space-y-0 lg:space-x-12">
            ...
            <div className="flex-shrink-0">
              <div className="relative">
                <Image
                  src={funeral.deceased.photo || "/placeholder.svg"}
            ...
            <div className="flex-1 text-white">
              <div className="flex flex-wrap items-center gap-4 mb-6">
                <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold">{funeral.deceased.name}</h1>
                {funeral.isUpcoming && (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                <div>
                  <p className="text-xl text-slate-300 mb-2">
                    {new Date(funeral.deceased.dob).toLocaleDateString()} -{" "}
                    {new Date(funeral.deceased.dod).toLocaleDateString()}
                  </p>
                  <p className="text-amber-300 font-semibold text-lg">
                    {calculateAge(funeral.deceased.dob, funeral.deceased.dod)} years of life
                  </p>
                </div>
                ...
                <div className="space-y-3">
                  <div className="flex items-center text-slate-300">
                    ...
                    <span>
                      {formatDate(funeral.funeral.date)} at {funeral.funeral.time}
                    </span>
                  </div>
                  <div className="flex items-center text-slate-300">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            ...
            <TabsContent value="overview" className="mt-8">
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
                <Card className="border-0 shadow-lg">
                  <CardHeader>
                    <CardTitle className="text-2xl text-slate-800">Life Story</CardTitle>
                  </CardHeader>
                  <CardContent className="space-y-6">
                    <p className="text-slate-700 leading-relaxed text-lg">{funeral.deceased.biography}</p>

                    <Separator />

                    <div>
                      <h3 className="text-xl font-semibold text-slate-800 mb-4">Family Information</h3>
                      <p className="text-slate-700 leading-relaxed mb-4">{funeral.family.details}</p>
                      <div className="flex items-center text-slate-600">
                        <Phone className="w-4 h-4 mr-2" />
                        <span>{funeral.family.contact}</span>
                      </div>
                    </div>
                    ...
                    <div>
                      <h3 className="text-xl font-semibold text-slate-800 mb-4">Service Details</h3>
                        ...
                        <div className="space-y-4">
                          ...
                          <div className="flex items-center p-4 bg-slate-50 rounded-xl">
                            ...
                            <div>
                              ...
                              <p className="text-slate-600">{funeral.funeral.venue}</p>
                            </div>
                          </div>
                        </div>
                        <div className="space-y-4">
                          ...
                          <div className="flex items-center p-4 bg-slate-50 rounded-xl">
                            ...
                            <div>
                              <p className="font-medium text-slate-800">Region</p>
                              <p className="text-slate-600">{funeral.funeral.region}</p>
                            </div>
                          </div>
                        </div>
                      </div>
                  ...
                  <CardContent>
                    <div className="space-y-6">
                      {funeral.condolences.map((condolence) => (
                        <div key={condolence.id} className="flex space-x-4 p-4 bg-slate-50 rounded-xl">
                          <Image
                            src={condolence.avatar || "/placeholder.svg"}
                            alt={condolence.name}
                            width={48}
                            height={48}
                          ...
                          <div className="flex-1">
                            <div className="flex items-center justify-between mb-2">
                              <div>
                                <p className="font-semibold text-slate-800">{condolence.name}</p>
                                <p className="text-sm text-slate-500">{condolence.location}</p>
                              </div>
                              <p className="text-xs text-slate-400">
                                {new Date(condolence.timestamp).toLocaleDateString()}
                              </p>
                            </div>
                            <p className="text-slate-700 leading-relaxed">{condolence.message}</p>
                          </div>
                        </div>
                      ))}
                    </div>
            ...
            <TabsContent value="support" className="mt-8">
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
                <DonationWidget
                  ...
                  recentDonations={funeral.donations.recent}
                />
              </motion.div>
            </TabsContent>
            ...
            <TabsContent value="gallery" className="mt-8">
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
                <Card className="border-0 shadow-lg">
                  <CardHeader>
                    <CardTitle className="text-xl text-slate-800">Photo Gallery</CardTitle>
                  </CardHeader>
                  <CardContent>
        ...
        <div className="space-y-8">
          ...
          >
            <Card className="border-0 shadow-lg">
              <CardHeader>
                <CardTitle className="text-lg text-slate-800">Community Support</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="grid grid-cols-2 gap-4">
                  <div className="text-center p-4 bg-green-50 rounded-xl">
                    <div className="text-2xl font-bold text-green-600">
                      {funeral.donations.currency} {funeral.donations.total.toLocaleString()}
                    </div>
                    <div className="text-sm text-green-700">Total Raised</div>
                  </div>
                  <div className="text-center p-4 bg-blue-50 rounded-xl">
                    <div className="text-2xl font-bold text-blue-600">{funeral.donations.supporters}</div>
                    <div className="text-sm text-blue-700">Supporters</div>
                  </div>
                  <div className="text-center p-4 bg-purple-50 rounded-xl">
                    <div className="text-2xl font-bold text-purple-600">{funeral.condolences.length}</div>
          ...
          >
            <Card className="border-0 shadow-lg">
              <CardHeader>
                <CardTitle className="text-lg text-slate-800">Funeral Poster</CardTitle>
              </CardHeader>
              <CardContent>
                <Image
                  src={funeral.poster || "/placeholder.svg"}
                  alt="Funeral poster"
                  width={400}
                  height={600}
          ...
          >
            <Card className="border-0 shadow-lg">
              <CardHeader>
                <CardTitle className="text-lg text-slate-800">Funeral Brochure</CardTitle>
              </CardHeader>
              <CardContent>
                <Button
            ...
            <Card className="border-0 shadow-lg">
              ...
              <CardContent>
                ...
                <div className="space-y-2">
                  <p className="font-medium text-slate-800">{funeral.funeral.venue}</p>
                  <p className="text-slate-600">{funeral.funeral.location}</p>
      ...
      {/* Flipbook Modal */}
      {showFlipbook && <FlipbookViewer pdfUrl={funeral.brochure} onClose={() => setShowFlipbook(false)} />}
    </div>
  )
}
